...
function clearRenderProduct(root_id){
...
function filter(){
    ...
    // Mới vào hiển thị tất cả sản phẩm
...
function renderProduct(products, addToCart, root_id){
    ...
    const htmlProduct = products.map((item) => {
        ...
        title.innerText = item.name[lang] || item.name.vi;
        ...
        buyBtn.addEventListener('click', (e) => {
            e.stopPropagation(); // Ngăn sự kiện click lan ra productCard
            addToCart(item);
        });
